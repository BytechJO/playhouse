function buildMcqBody(aObj) {	
	...
	if(typeof aObj !=undefined && aObj !=null){		
        ...
        htmlStmt += '<div class="h-100 d-flex flex-wrap justify-content-between align-items-center mb-70">';
			...
			for(y= 0;y<numOfQinCol;y++){
				...
				if(typeof tpOb != undefined && tpOb != null ){
                    ...
                    for (var rr = 0; rr < (opts).length; rr++) {
                        fillBoxesArr[rr]='';                       
						if(tpOb.inputbox == "no"){
							tmphtml = "<span class='pick_set d-flex' id='pick_set_"+currQueNum+"_"+(rr+1)+"'>";
						}else{
							tmphtml = "<span class='pick_set d-flex' id='pick_set_"+currQueNum+"_"+(rr+1)+"'><input class='text_input_area d-none' type='text' readonly disabled>";
						}						
                        var tmpOpt = opts[rr];
