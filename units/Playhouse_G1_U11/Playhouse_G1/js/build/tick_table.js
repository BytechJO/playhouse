function buildMcqTickBody(aObj) {
  ...
  if (aObj != undefined && aObj != null) {
    ...
    if (layOut == 1) {
      ...
      htmlStmt +=
        '<div class="tick_group rounded-corners d-flex flex-wrap justify-content-center align-items-center mb-70">';
      ...
      for (x = 0; x < numOfQuestions; x++) {
        ...
        htmlStmt +=
          ...
          '">';
        htmlStmt +=
          '<div class="que_group d-flex flex-wrap justify-content-center align-items-center">';
        htmlStmt += '<div class="d-flex justify-content-end icon_wrap_holder">';
        htmlStmt += '<div class="icon_wrap mx-1">';
        htmlStmt +=
          '<div class="tick iconcontainer"><img src="../images/icons/check_btn.png"/></div>';
        htmlStmt +=
          '<div class="cross iconcontainer"><img src="../images/icons/cross_btn.png"/></div>';
        ...
        htmlStmt +=
          '<table class="ans_grup que_group d-flex gap-20 justify-content-center" style="justify-content: space-evenly !important; margin-bottom: 50px;">';
  ...
  "_" +
  (y + 1) +
  '" class="tick_fieldgap-20 pick align-items-baseline"> ';
...
htmlStmt += tempObj.options[y].image
  ? `<img class="images-border" src="${tempObj.options[y].image}" alt="image"/>`
  : "";

var isFirstColumn = y % aObj.numInRow == 0;
var rowIndex = Math.floor(y / aObj.numInRow); // 0 = صف أول، 1 = صف ثاني، 2 = صف تالت...

if (isFirstColumn && rowIndex >= 2) {
  // ===== أول عامود، من الصف الثالث وطالع: انبوت كتابة =====
  htmlStmt += '<div class="writeBoxWrap">';
  htmlStmt +=
    '<input type="text" class="writeBoxInput" id="write_' +
    (x + 1) +
    "_" +
    (y + 1) +
    '" data-qno="' +
    ...
    '" autocomplete="off" />';
  htmlStmt += "</div>";
} else if (tempObj.options[y].tick_box == "no") {
  htmlStmt += tempObj.options[y].text.length
    ? '<div class="tickContent">' + tempObj.options[y].text + "</div>"
    : "";
} else {
  htmlStmt += '<div class="tickBox">';
  htmlStmt +=
    '<span class="selectTick" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>';
...
htmlStmt += "</td>"; // end - pick
        ...
        htmlStmt += tempObj.image
          ? '<div class="image_wrap"><img src="' + tempObj.image + '"></div>'
          : "";
