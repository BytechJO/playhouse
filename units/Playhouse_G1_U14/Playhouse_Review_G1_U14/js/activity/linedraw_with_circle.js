...
LineDraw.prototype = {
    ...
    listen:function(ob){
        ...
        ////////////////////////////////////// circle-option
        $area.find('.circle-option').click(function () {
            var $this = $(this);
            var parentDropId = $this.closest('.drop').attr('id');
            $area.find(`#${parentDropId} .circle-option`).removeClass('selected'); // Deselect others
            $this.addClass('selected'); // Select clicked option
        });
        ////////////////////////////////////// circle-option
    },
    setValuesForLine:function(lineOb){
        ...
        (lineOb).endX =((lineOb).endNode).position().left + (ed_margin_left - halfWidth) + 20 ;
    ...
    validate: function () {
        ...
        for (var dp1 = 0; dp1 < numOfDrops; dp1++) {
            ...
            // **New Validation: Circle Option**
            var userSelectedOption = $area.find(`#drop_${dp1 + 1} .circle-option.selected`).text();
            var correctCircleAnswer = dataQuestion.drops[dp1].circleanswer;
            var isCircleCorrect = userSelectedOption === correctCircleAnswer;

            if (thisIsCorr && isCircleCorrect) {
                ob.correctCount++;
                ob.resultArr[dp1] = 1;
            } else {
                ob.wrongCount++;
    ...
    resetCircles: function() {
        ...
        // Iterate through all the drops
        $area.find('.drop').each(function() {
            // Find the circle options within each drop
            $(this).find('.circle-option').each(function() {
                // Reset any selection or styles for the options
                $(this).removeClass('selected'); // Assuming 'selected' is the class used to indicate selection
            });
        });
    },
