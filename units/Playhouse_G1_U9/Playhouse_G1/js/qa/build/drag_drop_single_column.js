function initActivity(activity){
	...
	//Questions
	drag_drop_questions = '<div class="drag_drop_questions center_item"><ul class="que_group">';
	jQuery.each(activity.questions, function(key, values){
		drag_drop_questions += '<li class="que que_'+key+'"><ul class="text d-flex flex-wrap justify-content-start">';
		if(typeof(values)=="string"){
			...
			jQuery.each(values, function(k, v){
				var v = v+"";
				drag_drop_questions += '<li class="drag_drop_multiple" style="margin:27px 20px; height: 30px;">'+ v.replace('___', ' <input readonly type="text" class="droppable_div" />') +'</li>';
			});
		}
	...
	for(let x=0; x < activity.images.length; x++){
		html += '<img src="'+activity.images[x]+'" class="q_img img-'+x+'"/>';
	}
