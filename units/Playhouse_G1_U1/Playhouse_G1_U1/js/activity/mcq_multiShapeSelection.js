...
MCQ.prototype = {
    ...
    listen: function (ob) {
		...
		ob.selectedShape = '';
		if($area.find('.btn_sel_shape').length > 0){
			$area.find('.btn_sel_shape').each(function(){
				if($(this).hasClass('selectedShape')){					
					ob.selectedShape = $(this).data('shape');
				}
			});
				
			$area.find('.btn_sel_shape').click(function(){
				var _isSelectedShape = $(this).hasClass('selectedShape');
				$area.find('.btn_sel_shape').removeClass('selectedShape');
				if(!_isSelectedShape){
					$(this).addClass('selectedShape');
					ob.selectedShape = $(this).data('shape');
				}
				console.log('ob.selectedShape :sel: ', ob.selectedShape);
			});
		}
		console.log('ob.selectedShape :: ', ob.selectedShape);
		var selShape = ob.selectedShape;
        ...
        if (ques.length > 0) {
            ...
            for (var i = 0; i < picks.length; i++) {
                picks[i].addEventListener("click", function (e) {
                    var target = $(e.target);
					selShape = ob.selectedShape;
                    console.log('target .. ',selShape);
                    ...
                    if (isSel) {
						...
						//$(this).data('shapeadded', selShape);
						$(this).addClass(selShape);
                        if (selShape == 'cross') {
                            $(this).find('.selX').show();
    ...
    resetAllPicks: function (qOb, aArr) {
        ...
        for (var i = 0; i < picks.length; i++) {
			...
			(tPick.querySelector('.txt_box')).classList.remove("underLineIt");
    ...
    validate: function () {
        ...
        for (var i = 0; i < elsQue.length; i++) {
			...
			var _circleAns = getIntArray(fDataObj.circle);
			var _underlineAns = getIntArray(fDataObj.underline);
			
			var _userCircleAns = [];
			var _userUnderlineAns = [];

            var picks = elsQue[i].querySelectorAll('.pick');
            if (picks.length > 0) {
                for (var a = 0; a < picks.length; a++) {
					//$(this).data('shapeadded', selShape);
					console.log('> ',a, picks[a].classList.contains('circle'), picks[a].classList.contains('underline'));
                    if (picks[a].classList.contains('selected')) {
						var _isShaped = ( picks[a].classList.contains('circle'))? 'circle' : (picks[a].classList.contains('underline'))?'underline':'none';
                        //picks[a].classList.remove('selectedDefault')
                        _uAns.push(a + 1);
                        var isAns = ($.inArray((a + 1), _cAns) >= 0);
						if(_isShaped == 'circle'){
							_userCircleAns.push(a + 1);
						}else if(_isShaped == 'underline'){
							_userUnderlineAns.push(a + 1);
						}
                        /*if(selShape == 'tickbox'){
			...
			console.log(i, _uAns, _cAns, _userCircleAns, _circleAns, _userUnderlineAns,_underlineAns);
            if ((_uAns.length > 0) && (_cAns.length == _uAns.length)) {
                var  res1 = (compareArrays(_uAns, _cAns));
				 var  res2 = (compareArrays(_userCircleAns, _circleAns));
				  var  res3 = (compareArrays(_userUnderlineAns,_underlineAns));
				resultArr[i] = (res1 && res2 && res3)?1:0;
                if ((fDataObj).audio != '') {
    ...
    reset: function () {
        ...
        for (var i = 0; i < elsQue.length; i++) {
            ...
            if (pickOptions.length > 0) {
                for (var a = 0; a < pickOptions.length; a++) {
					pickOptions[a].dataset.shapeadded = "none";
		...
		$area.find('.pick').each(function(){
			//$(this).data('shapeadded', '');
			$(this).removeClass('circle');
			$(this).removeClass('underline');
		});
		/*if($area.find('.btn_sel_shape').length > 0){
