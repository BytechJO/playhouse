    ...
    function buildFillInBody(aObj) {		
		...
		if(aObj !=undefined && aObj !=null){
			...
			if(layOut == 1){
				...
				var rowCount = 1;
				for(x= 0;x<numOfRows;x++){
					htmlStmt += '<div class="flex-column row_'+rowCount+' d-flex flex-wrap justify-content-center">';
					for(y= 0;y<numInRowArray[x].length;y++){
						htmlStmt += '<div class="que d-flex align-items-center gap-80 justify-content-between f_i_t_2_q_group f_i_t_2_g_'+(y+1)+'" data-qno="'+(currentQue)+'">';
						htmlStmt += `<div class="que-left d-flex">`
							htmlStmt += '<div class="img_box1"><img src="'+((aObj.questions)[currentQue-1]).image+'"></div>';
								...
								htmlStmt += `<span>${((aObj.questions)[currentQue-1]).sentenceFirst ? ((aObj.questions)[currentQue-1]).sentenceFirst : ''}</span>`
								htmlStmt += '<div class="audioIcon off disabled" data-audio="'+((aObj.questions)[currentQue-1]).audio+'"></div>';
								if(((aObj.questions)[currentQue-1]).inputFirst){
									htmlStmt += `<input class="mx-2 input-line" type="text" maxlength="10"> ${((aObj.questions)[currentQue-1]).wordAfterInput ? ((aObj.questions)[currentQue-1]).wordAfterInput : ''}`
								}
							htmlStmt += '</div>';
							htmlStmt += '</div>';
							htmlStmt += `<div class="que-right">`
								htmlStmt += '<div class="txt_box d-flex justify-content-center align-items-center" data-type="text">';
								htmlStmt += '<div class="audioIcon off disabled" data-audio="'+((aObj.questions)[currentQue-1]).audio+'"></div>';
								htmlStmt += "<span> It's </span>";
										htmlStmt += '<input class="mx-2 input-line" type="text" maxlength="10">'
										htmlStmt += '<input class="mx-2 input-line" type="text" maxlength="10">'
