function buildMcqBody(aObj) {	
	...
	if(typeof aObj !=undefined && aObj !=null){		
		...
		for(x= 0;x<numberofCols;x++){	
			htmlStmt += '<div class="question_group question-container">';			
			for(y= 0;y<numOfQinCol;y++){
				currQueNum++;
				var tpOb = (aObj.questions)[currQueNum -1];
				if(typeof tpOb != undefined && tpOb != null ){
					htmlStmt += '<div class="que delete-border" id="que_'+currQueNum+'" data-qno="'+currQueNum+'">';
					...
					var classImage = aObj.imageClass !== 'yes' ? 'image-question' : ''
                  	if(tpOb.image != '' && tpOb.image != 'no'){
						 htmlStmt += `<div class="img_space ${classImage}"><img src=${tpOb.image}></div>`;
					}
                    ...
                    for (var rr = 0; rr < (opts).length; rr++) {
						...
						if(tpOb.inputbox == "no"){
							...
							tmphtml = "<span class='pick_set d-flex flex-column' id='pick_set_"+currQueNum+"_"+(rr+1)+"'><input class='d-none text_input_area' type='text' readonly disabled>";
						}						
                        // tmphtml += "&nbsp;(&nbsp;";
                        var tmpOpt = opts[rr];
                    ...
                    htmlStmt += '<div class="h-100">'+str+'</div>';
