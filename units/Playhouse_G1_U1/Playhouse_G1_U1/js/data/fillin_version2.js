//  ****************************************** //
//  FillIn - Version no: 1.3
//  Date updated - June 3, 2020 
//  Date updated - August 12, 2020 (validation revamped)
//  Date updated - August 16, 2020 (with answer groups) 
//  ****************************************** //
...
FillIn.prototype = {
    ...
    validate:function(){
        ...
        var resultArr = []; 
		// added - groupAnsValidation - Aug 16th, 2020
		var groupAnsValidation= 'no';
		var groupDoneAnswers = [];
		if(typeof (ob.data_obj).answergroups != undefined && (ob.data_obj).answergroups!=null ){
			groupAnsValidation = 'yes';
			groupDoneAnswers = [];
		}
        $area.find('.que').each(function(){
			...
			if(groupAnsValidation == 'yes' && ((ob.data_obj).answergroups).length >0){
				_ingroup = (fDataObj.answergroup != undefined && fDataObj.answergroup!=null )? parseInt(fDataObj.answergroup):0;
				if(_ingroup > 0){
					_cAns = (ob.data_obj).answergroups[_ingroup -1];
				}
			}
			console.log('groupAnsValidation >> ', groupAnsValidation, _ingroup, _cAns);
            var numOfInputs = $(this).find('input').length; 
            var doneAnswers = [];
            if(_case == 'no'){
				for(var aa=0;aa<_cAns.length;aa++){
					_cAns[aa] = ($.trim(_cAns[aa])).toLowerCase();
				}
			}
            $(this).find('input').each(function(){
                if($(this).attr('readonly') == null && $(this).attr('disabled') == null){
                    ...
                    if(thisVal != ""){
						...
						if(groupAnsValidation == 'yes'){
                            
                            var isInAnswers = ($.inArray(thisVal, _cAns) > -1);
                            var isAlreadyDone = ($.inArray(thisVal, groupDoneAnswers) > -1);
                            if(isInAnswers && !isAlreadyDone){
                                groupDoneAnswers.push(thisVal);
                                _uAns[thisInd] = 1; 
                            }
                             console.log('>groupAnsValidation> ',  thisQNum, thisInd,thisVal, isInAnswers, isAlreadyDone, groupDoneAnswers);
                        }else {
							if((numOfInputs > 1 && _validationOrderCase == 'no')||(groupAnsValidation == 'yes')){
								...
								if(isInAnswers && !isAlreadyDone){
								 ...
								 console.log('>groupAnsValidation> ',  thisQNum, thisInd,thisVal, isInAnswers, isAlreadyDone, doneAnswers);
							}else{
								//console.log('>_validationOrderCase - yes> ', thisVal, thisInd, _cAns[thisInd]);
			...
			//console.log('>_validationOrderCase - yes> ', _uAns, resultArr);
        }); 
