	...
	if(typeof(activity_type)!='undefined'){
		...
		jQuery('.'+activity_type+'_activity input[type="text"]').each(function(k, v){
			if(_activity_json.answers[k]!=jQuery(this).val()){
				jQuery(this).parent().find('.selected').append('<span class="activity_result"><img src="../images/icons/cross_btn.png" /></span>');
				err++;
			} else {
				jQuery(this).parent().find('.selected').append('<span class="activity_result"><img src="../images/icons/check_btn.png" /></span>');
			}
		});
