...
function randomH() {
  ...
  var randomColor = Colorpalette.prototype.HSVtoRGB(
    360 * Math.random(),
    90,
    100,
  );
    ...
    if (bulkFrame.evs && bulkFrame.evs.length) {
      while (bulkFrame.evs.length) {
        ...
        for (i = bulkFrame.evs.length - 1; i >= 0; i -= 1) {
          ...
          if (
            !bulkFrame.evs[i].pts ||
            (bulkFrame.evs[i].pts && bulkFrame.evs[i].pts.t.length < 2)
          ) {
...
Object.assign(ToolAipen.prototype, {
  ...
  updateBestFigure(input) {
    ...
    let prevPoint;
    const lineDistance = centeredInputRect.centeredInput.reduce(function (
      previousValue,
      currentValue,
      index,
      array,
    ...
    var firstLastDistance = this.distance(
      firstPoint.x,
      firstPoint.y,
      lastPoint.x,
      lastPoint.y,
    ...
    for (rotation = 0; rotation < 360; rotation += 1) {
      var boundingInputRect = this.getBoundingInputRect(
        this.rotateInput(centeredInputRect.centeredInput, rotation),
      );
      boundingInputRect.rotation = rotation;
      projections.angle[rotation] = boundingInputRect;
      if (
        projections.optimal === undefined ||
        ...
        if (
          projections.optimal &&
          projections.optimal.area === boundingInputRect.area &&
          projections.optimal.width === boundingInputRect.width &&
          1
        ) {
          console.log("bingo");
        }
      ...
      if (
        projections.horizontal === undefined ||
      ...
      if (
        projections.vertical === undefined ||
    ...
    projections.isClosed =
      firstLastDistance <
      Math.sqrt(projections.maxWidth * projections.maxWidth * 0.05);
    console.log(
      "%cPROJECTIONS",
    ...
    if (detections.length) {
      detections.sort(function (a, b) {
        return a.missmatch - b.missmatch;
      });
      console.groupCollapsed(
        `Detections [${detections[0].detector}] ${detections[0].missmatch}`,
      );
      ...
      if (bestDetection && bestDetection.missmatch < 0.05) {
        setTimeout(function () {
          ...
          that.drawProjector({
            ...
            prePoints: [
              {
                x: centeredInputRect.pointsWage.x,
                y: centeredInputRect.pointsWage.y,
                t: 1,
              },
              {
                x: centeredInputRect.pointsWage.x,
                y: centeredInputRect.pointsWage.y + 1,
                t: 2,
              },
            ],
          });
          that.drawProjector({
            ...
            prePoints: [
              {
                x: centeredInputRect.centerX,
                y: centeredInputRect.centerY,
                t: 1,
              },
              {
                x: centeredInputRect.centerX,
                y: centeredInputRect.centerY + 1,
                t: 2,
              },
            ],
          });
          ...
          that.drawProjector({
            ...
            prePoints: [
              {
                x: bestDetection.boundingInputRect.pointsWage.x,
                y: bestDetection.boundingInputRect.pointsWage.y,
                t: 1,
              },
              {
                x: bestDetection.boundingInputRect.pointsWage.x,
          ...
          that.drawProjector({
            ...
            prePoints: [
              {
                x: bestDetection.boundingInputRect.centerX,
                y: bestDetection.boundingInputRect.centerY,
                t: 1,
              },
              {
                x: bestDetection.boundingInputRect.centerX,
          ...
          that.drawProjector({
            ...
            prePoints: [
              {
                x: bestDetection.boundingInputRect.cMinX,
                y: bestDetection.boundingInputRect.cMinY,
                t: 1,
              },
              {
                x: bestDetection.boundingInputRect.cMaxX,
                y: bestDetection.boundingInputRect.cMinY,
                t: 2,
              },
              {
                x: bestDetection.boundingInputRect.cMaxX,
                y: bestDetection.boundingInputRect.cMaxY,
                t: 3,
              },
              {
                x: bestDetection.boundingInputRect.cMinX,
        ...
        input.viewportX +=
          (centeredInputRect.centerX +
            bestDetection.boundingInputRect.centerX) /
          input.scale;
        input.viewportY +=
          (centeredInputRect.centerY +
            bestDetection.boundingInputRect.centerY) /
          input.scale;
  ...
  getDistanceFromLine(p1, p2, p) {
    var A = p2.x - p1.x,
      ...
      p3;
    var u =
      (A * (p.x - p1.x) + B * (p.y - p1.y)) / (Math.pow(A, 2) + Math.pow(B, 2));
    if (u <= 0) {
      p3 = p1;
    } else if (u >= 1) {
      p3 = p2;
  ...
  detectLine(projections, boundingInputRect) {
    ...
    for (i = 0; i < boundingInputRect.centeredInput.length; i += 1) {
      ...
      var missmatch = this.getDistanceFromLine(
        { x: boundingInputRect.cMinX, y: 0 },
        { x: boundingInputRect.cMaxX, y: 0 },
        point,
      );
      missmatchSum += missmatch;
    }
    const perimeter = boundingInputRect.width;
    const perimeterMissmatch =
      Math.abs(projections.lineDistance - perimeter) / perimeter;
    const distanceMissmatch =
      missmatchSum /
      boundingInputRect.centeredInput.length /
      boundingInputRect.avgEdge;
  ...
  detectTriangle(projections, boundingInputRect) {
    ...
    let missmatchSum = 0;
    var a = boundingInputRect.pointLeft,
      b = boundingInputRect.pointRight,
      c = boundingInputRect.pointTop;

    const perimeter =
      this.distance(a.x, a.y, b.x, b.y) +
      this.distance(b.x, b.y, c.x, c.y) +
      this.distance(c.x, c.y, a.x, a.y);
    if (!perimeter) {
      return;
    }
    ...
    const perimeterMissmatch =
      Math.abs(projections.lineDistance - perimeter) / perimeter;
    const distanceMissmatch =
      missmatchSum / centeredInput.length / boundingInputRect.avgEdge;
  ...
  detectRectangle(projections, boundingInputRect) {
    ...
    for (i = 0; i < centeredInput.length; i += 1) {
      point = centeredInput[i];
      var a = { x: boundingInputRect.cMinX, y: boundingInputRect.cMinY },
    ...
    const perimeter =
      boundingInputRect.width * 2 + boundingInputRect.height * 2;
    if (!perimeter) {
      return;
    }
    const perimeterMissmatch =
  ...
  detectEllipse(projections, boundingInputRect) {
    ...
    for (i = 0; i < centeredInput.length; i += 1) {
      ...
      missmatch = this.distance(
        input.x,
        input.y,
        closestPointOnEllipse.x,
        closestPointOnEllipse.y,
      );
      missmatchSum += missmatch;
    }
  ...
  detectCircle(projections, boundingInputRect) {
    ...
    const avgDistancesFromCenter =
      sumDistancesFromCenter / centeredInput.length;
    let currentDistance;
    let missmatchSum = 0;
    for (i = 0; i < centeredInput.length; i += 1) {
      input = centeredInput[i];
    ...
    if (
      missmatchAvg >
      Math.sqrt(boundingInputRect.width * boundingInputRect.height)
    ) {
      isCircle = false;
    }
    ...
    console.log(
      `%cisCircle [%c${isCircle}]`,
      "background:black;color:white",
      "background:black;color:white",
      "P:",
      perimeter,
      "L:",
      projections.lineDistance,
      "E:",
      perimeterMissmatch,
    );
...
function Sketchpad(config) {
  ...
  if (NSSketchpad.watermarkImageSrc) {
    ...
    this.watermark.addEventListener("click", function () {
      ...
      var qrCodeCanvas =
        qrCode[String("_" + "oDrawing")][String("_" + "elCanvas")];
  ...
  this.syncPointerDataFrequency =
    config.syncPointerDataFrequency || Math.round(1000 / 20);
...
Object.assign(Sketchpad.prototype, {
  ...
  hideGrid: function hideGrid() {
    "use strict";
    this.displayGrid = false;
    this.contextProjector2D.clearRect(
      -this.width / 2,
      -this.height / 2,
      this.width,
      this.height,
  ...
  syncCanvasFrame: function syncCanvasFrame() {
    ...
    if (
      ...
      this.planRefreshWindow(
        RESIZE_REFRESH_BOUNCE_TIME,
        "room.js:syncCanvasFrame:(resize)",
      );
    }
    if (this.receivingHistory) {
      this.loadingEl.textContent =
      ...
      case "nick-changed":
        if (obj.user_id && obj.user_name) {
          ...
          if (
            this.user &&
            this.user.user_id &&
            String(this.user.user_id) === obj.user_id
          ) {
