...
function buildFillInBody(aObj) {		
	...
	if(aObj !=undefined && aObj !=null){
		...
		if(layOut == 1){
			...
			htmlStmt += '<div class="options cont_ht_sf mx-auto"><div class="all_cont justify-content-center"><div class="cont_group">';
			var rowCount = 1;
			for(x= 0;x<numOfRows;x++){
				htmlStmt += '<div class="row_'+rowCount+' d-flex flex-wrap justify-content-center que-container">';
				for(y= 0;y<numInRowArray[x].length;y++){
					htmlStmt += '<div class="que f_i_t_2_q_group f_i_t_2_g_'+(y+1)+' question" data-qno="'+(currentQue)+'">';					
						...
						htmlStmt += '<img src="'+((aObj.questions)[currentQue-1]).imageRight+'">';
						htmlStmt += '</div>';
			...
			htmlStmt += '<div class="main_title_container">';
				...
				if(aObj.main_title_text.length > 1) {
					for(let x=0; x < aObj.main_title_text.length; x++){
						htmlStmt += '<div class="audioIcon textEnd off d-flex contant" data-audio="'+aObj.main_title_audio+'">' ;
				...
				for(let i = 0; i < aObj.items.length; i++){
					if (i==0){
						htmlStmt += '<div class="d-flex imgs_sides" style="width:40%">';
							htmlStmt += "<img src='" + aObj.items[i].text_img + "' class='readHighlightsBtn imgToggle' data-img='showImg1'>";
							htmlStmt += "<img src='" + aObj.items[i].img + "' class='text_img showImg1 audioIcon off' data-audio='"+aObj.items[i].audio+"'>";
						htmlStmt += '</div>';
					}else if (i==1){
						htmlStmt += '<div class="middle-text" style="width:21%">';
							htmlStmt += "<img src='" + aObj.items[i].img + "' class='text_img showImg0'>";
							htmlStmt += "<span class='text audioIcon off' data-audio='"+aObj.items[i].audio+"'>" + aObj.items[i].text + "</span>";
						htmlStmt += '</div>';
					}else if (i==2){
						htmlStmt += '<div class="d-flex imgs_sides" style="width:39%;">';
							htmlStmt += "<img src='" + aObj.items[i].img + "' class='text_img showImg2 audioIcon off' data-audio='"+aObj.items[i].audio+"'>";
							htmlStmt += "<img src='" + aObj.items[i].text_img + "' class='readHighlightsBtn imgToggle' data-img='showImg2' style='margin-left: auto;'>";
						htmlStmt += '</div>';
					}
				}
			htmlStmt += '</div>';
