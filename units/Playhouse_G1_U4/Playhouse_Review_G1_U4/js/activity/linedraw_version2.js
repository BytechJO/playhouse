...
//  Date updated - [current] (fixed coordinate calculation - relative to .lines container)
...
LineDraw.prototype = {
  ...
  setValuesForLine: function(lineOb) {
    ...
    // استخدم activity_area كـ reference مش .lines
    var $area = $(ob.activity_area);
    ...
    var startNodeOffset = (lineOb.startNode).offset();
    var endNodeOffset   = (lineOb.endNode).offset();

    var endHalfWidth = parseInt((lineOb.endNode).outerWidth()) / 2;

    (lineOb).startX = startNodeOffset.left - areaOffset.left + halfWidth;
    (lineOb).startY = startNodeOffset.top  - areaOffset.top  + halfWidth;

    (lineOb).endX = endNodeOffset.left - areaOffset.left + endHalfWidth;
    (lineOb).endY = endNodeOffset.top  - areaOffset.top  + endHalfWidth;

    (lineOb).distance = Math.sqrt(
        Math.pow((lineOb).endX - (lineOb).startX, 2) +
        Math.pow((lineOb).endY - (lineOb).startY, 2)
    );
    (lineOb).distance += (lineOb).height;
    (lineOb).angle = Math.atan2(
  ...
  drawRect: function(lineOb, axn, nam) {
    ...
    $lines.find('svg[data-line="' + obj.name + '"]').remove();

    var cp1x = (obj.startX + obj.endX) / 2;
    var cp1y = obj.startY;
    var cp2x = (obj.startX + obj.endX) / 2;
    var cp2y = obj.endY;

    $lines.append(
        '<svg data-line="' + obj.name + '" width="1" height="1" ' +
            'style="position:absolute;top:0;left:0;overflow:visible;pointer-events:none;">' +
            '<path id="' + obj.name + '" ' +
                'd="M ' + obj.startX + ' ' + obj.startY +
                ' C ' + cp1x + ' ' + cp1y +
                ', '  + cp2x + ' ' + cp2y +
                ', '  + obj.endX + ' ' + obj.endY + '" ' +
                'stroke="' + ob.lineColor + '" stroke-width="3" ' +
    ...
    validate:function(){ 
        ...
        for(var ll=0;ll<(ob.lineObjects).length; ll++){
            ...
            if(correctAns.length > 0){
                ...
                $area.find('#'+tOb.name).css({"stroke": tOb.color});
            }
        }
