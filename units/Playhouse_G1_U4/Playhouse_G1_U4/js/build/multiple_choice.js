function buildMcqBody(aObj) {	
	...
	if(typeof aObj !=undefined && aObj !=null){		
				...
				if(typeof tpOb != undefined && tpOb != null ){
					...
					if((tpOb.options).length > 0){
                        for(var opt=0;opt<(tpOb.options).length;opt++){
							...
							htmlStmt += '<div class="img_container"><img src="'+((tpOb.options)[opt]).image+'"/></div>';                               
                            htmlStmt += '</div>';
                        }
		...
		if(aObj.image2 != 'no' && aObj.image2 != ''){
            if(aObj.image2_position == 'back'){
                htmlStmt += '<div class="img_space"><img src="'+aObj.image2+'"></img></div>';
            }
        }
