		...
		for (x = 0; x < numOfRows; x++) {
			...
			for (y = 0; y < numInRowArray[x].length; y++) {
				...
				htmlStmt += '<div class="txtBox" data-type="text">';
					htmlStmt += '<div class="txt-audioIcon off d-flex contant min_w_fit_contant" data-audio="' + ((aObj.questions)[currentQue - 1]).audio + '">';
						htmlStmt += ((aObj.questions)[currentQue - 1]).textfront;
					htmlStmt += '</div>';

					if(((aObj.questions)[currentQue - 1]).textmiddle != "" && ((aObj.questions)[currentQue - 1]).textmiddle != undefined)
					{
						htmlStmt +=  inputboxstmt;
						htmlStmt += '<div class="txt-audioIcon off d-flex contant min_w_fit_contant" data-audio="' + ((aObj.questions)[currentQue - 1]).audio + '">';
							htmlStmt += ((aObj.questions)[currentQue - 1]).textmiddle;
						htmlStmt += '</div>';
						htmlStmt +=  inputboxstmt;
					}else {
						htmlStmt +=  inputboxstmt;
